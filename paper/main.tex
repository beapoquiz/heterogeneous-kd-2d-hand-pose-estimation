...
\author{
\IEEEauthorblockN{Bea Juliana B. Poquiz \quad Rhandley D. Cajote, Ph.D. \quad Dale Joshua R. Del Carmen}
\IEEEauthorblockA{\textit{Digital Signal Processing Laboratory} \\
...
\node (loss) [startstop, below right=of targets,
    xshift=0.2cm, minimum width=4.7cm]
    {\textbf{Distillation Loss} (Wing $+$ Bone)};
\node (output) [startstop, below=of loss]
...
\draw [arrow] (decoder) -- node[right, font=\tiny, align=left]
    {Taylor\\Expansion} (targets);
\draw [arrow] (targets) -- (loss.north -| targets.south);
\draw [arrow] (student) -- (pred);

% --- THE FOOLPROOF ARROW FIX ---
% This forces the line to start from the physical bottom edge of 'pred'
% and explicitly points downward into the 'loss' block.
\draw [arrow] (pred.south) ++(0,0.15cm) -- (loss.north -| pred.south);
% -------------------------------
...
Two MediaPipe-related references are kept strictly distinct. The
\textbf{official MediaPipe Hands SDK} (\texttt{mediapipe} pip
package, closed-source weights) is a domain-gap reference, not a
trained competitor: it was trained on real-world and photorealistic
imagery rather than RHD2D's flat-shaded synthetic
style~\cite{mediapipe2020}, so any score gap reflects a \emph{domain
gap}, not an architectural limitation. The \textbf{BlazeHandLandmark
student} was instead initialized from the \textbf{zmurez
MediaPipePyTorch port}~\cite{mediapipepytorch2021}, an open
reimplementation with converted weights, separating the
architecture's own starting point from the closed SDK's
out-of-domain performance.

Detection failure is defined per system: for the SDK, its internal
BlazePalm detector and landmark model jointly return no hand; for the
zmurez weights and the distilled student -- which never call
BlazePalm, evaluating directly on the fixed $256{\times}256$ crop --
the model's own \texttt{hand\_flag} head scores $\leq0.5$. Failures
are scored rather than excluded: every keypoint on a failed frame is
assigned a worst-case 256\,px error, more conservative than published
MediaPipe benchmarks, which score detected frames only. Scored
separately, the SDK detects 66.2\% of frames (PCK@0.2\,=\,0.901
detected-only) and the zmurez weights 80.1\% (0.783); both localize
substantially better than their all-frames figures suggest.
...
Table~\ref{tab:v1v2} summarises the design improvements introduced
in V2 to address the overfitting and instability observed in V1;
these are the configuration evaluated throughout the rest of this
paper. Figure~\ref{fig:learning_curves} shows V2 validation metrics
...
\begin{axis}[
  name=ax3,
  at={(ax1.south west)}, yshift=-1.2cm,
  anchor=north west,
  width=0.44\columnwidth, height=2.8cm,
  xlabel={\scriptsize Epoch},
...
annotations; the student architecture's initialization state
\emph{prior} to distillation is quantified separately below.

The parity-controlled direct-supervision retrain (protocol v2match)
reverses this paper's original comparison: it reaches
PCK@0.2\,=\,0.970, AUC\,=\,0.894, and MPJPE\,=\,13.63\,px
(Table~\ref{tab:trained}), outperforming both distilled models by
12.6--12.7\,pp AUC. Because AUC integrates PCK over thresholds as
tight as 5\,px, the gap holds at fine-grained spatial precision, not
only at the lenient 51.2\,px PCK@0.2 window. The original
direct-supervision baseline (PCK\,=\,0.425, AUC\,=\,0.538) was never
a fair comparison: it read targets from the wrong coordinate frame
(mean 69.7\,px offset in a 256\,px-wide space) and omitted the Bone
loss used throughout the distillation arm -- artifacts of an
incomplete implementation, not a limitation of direct supervision.
Corrected, and given the identical recipe already used for
distillation (Table~\ref{tab:v1v2}), it wins outright; that recipe
was tuned only against the distillation arm, so the result should be
read as ``under an identical, distillation-derived recipe,'' not as
direct supervision at its own best settings. The reversal is
mechanistically unsurprising: the teacher itself reaches only
AUC\,=\,0.902 against ground truth, so a distilled student inherits
the teacher's error on top of its own, while direct supervision has
only the latter to overcome -- and RHD2D's pixel-perfect synthetic
labels remove the label-scarcity problem that normally offsets this
for distillation. The corrected baseline also shows no overfitting
through epoch~100 (the epoch-90 checkpoint scores within 0.01\,pp AUC
of the final one). Within the distillation arm, Stage~2 fine-tuning
adds a modest further gain over Stage~1 ($+$0.002 AUC, $-$0.21\,px
MPJPE; PCK@0.2 statistically tied), indicating Phase~1 captures the
bulk of transferable knowledge while fine-tuning serves as
convergence refinement. The teacher remains the accuracy upper bound
at AUC\,=\,0.902 but is not a deployment candidate at 6.9\,FPS.
...
\multicolumn{8}{l}{\scriptsize variance, \S\ref{sec:conclusion}); best of the two
...
\multicolumn{8}{l}{\scriptsize term (\S\ref{sec:quant}), for all rows shown.}\\
...
Two initialization states of the same architecture, both scored
against GT on the full test set, contextualise the pipeline's
contribution: random weights score PCK@0.2\,=\,0.001, AUC\,=\,0.014,
MPJPE\,=\,188.88\,px at 0.00\% detection, while the zmurez
BlazeHandLandmark weights~\cite{mediapipepytorch2021}
(pre-distillation, no ground truth or teacher signal; distinct from
the official SDK in Table~\ref{tab:consolidated}, \S V-A) score
0.627, 0.589, 78.25\,px, 80.05\%. The rise from PCK@0.2\,=\,0.627 to
0.819 after distillation (Table~\ref{tab:trained}, FT~V2 ep.\,150)
quantifies the framework's contribution to localization accuracy.
The accompanying rise in detection rate (80.05\%\,$\to$\,100\%) is
\emph{not} part of it, and is withdrawn as a claimed strength:
tracing the training code shows \texttt{hand\_flag} never appears in
any loss term, so its weights receive no gradient update while the
backbone is heavily retrained by the coordinate-regression
objective, leaving a frozen classification head reading features it
was never recalibrated for -- a numerically correct but
uninformative artifact, not evidence of detection capability. The
zmurez row's 80.05\% is a genuine, discriminating value from the
same code path, so the metric itself is sound; only the fine-tuned
checkpoint's head has lost its signal. One caveat attaches to the
localization gain itself: all three trained-model rows were trained
and evaluated on BGR-ordered pixels (an inherited OpenCV default)
while the zmurez and SDK rows were correctly RGB-converted, so an
unquantified channel-order confound accompanies the distillation
effect -- flagged rather than corrected here
(\S\ref{sec:conclusion}). These results are not a critique of
MediaPipe Hands or the zmurez port on their intended domain; they
isolate the domain-adaptation problem this open, retrainable
framework addresses.
...
efficient GPU utilisation for batched video pipelines
(Table~\ref{tab:throughput}). MediaPipe Hands Full's reported
5.3--16.1\,ms on Pixel~3/S20/iPhone~11~\cite{mediapipe2020} is not
comparable here, but indicates the BlazeHandLandmark architecture is
itself well-matched to mobile deployment.
...
trained models on the full RHD2D test set. Its Direct Sup.\ column
reflects the original, uncorrected implementation and was not re-run
for the parity-controlled baseline (\S\ref{sec:quant}), so it should
not be read against the corrected aggregates in
Table~\ref{tab:trained}; a corrected per-joint comparison is left to
future work. Within this table's own data, FT~V2-150 leads on 12 of
21 joints and Dist.\ V2-100 on the remaining 9, and both distilled
variants outperform the superseded Direct Sup.\ baseline on all 21
joints by 0.20--0.65 PCK@0.2. That improvement was not uniform: it
tracked how poorly the uncorrected baseline localized each joint to
begin with, concentrating its largest gains at the wrist
(PCK@0.2\,=\,0.204, $+$0.646 for FT~V2-150) and MCP (mean
0.338, $+$0.537), and its smallest at Tip (0.472, $+$0.270) and PIP
(0.432, $+$0.405).

A separate gradient holds within the distilled models themselves:
Tip is the lowest-scoring joint of every finger, and the group means
order Tip (0.742) $<$ DIP (0.816) $<$ PIP (0.836) $<$ MCP (0.875)
for FT~V2-150, with Dist.\ V2-100 ordering identically. An earlier
draft attributed this to teacher heatmap confidence; that explanation
has not been empirically validated -- no teacher heatmap
spatial-spread analysis is currently reproducible from this
project's codebase -- so we do not assert a mechanism here. A
kinematic or loss-weighting factor independent of heatmap confidence
remains a plausible candidate, pending a properly instrumented and
version-controlled analysis (\S\ref{sec:conclusion}).
...
\textbf{Digit} & MCP & PIP & DIP & Tip & MCP & PIP & DIP & Tip
               & MCP & PIP & DIP & Tip \\
\midrule
Thumb  & 0.284 & 0.364 & 0.423 & 0.386
       & \textbf{0.899} & \textbf{0.836} & 0.819 & 0.762
       & 0.897 & 0.835 & \textbf{0.821} & \textbf{0.766} \\
Index  & 0.326 & 0.402 & 0.457 & 0.412
       & \textbf{0.902} & \textbf{0.854} & 0.805 & 0.745
       & 0.897 & 0.848 & \textbf{0.806} & \textbf{0.747} \\
Middle & 0.329 & 0.449 & 0.602 & 0.582
       & \textbf{0.908} & \textbf{0.879} & 0.838 & \textbf{0.786}
       & 0.906 & 0.879 & \textbf{0.840} & 0.781 \\
Ring   & 0.367 & 0.490 & 0.555 & 0.504
       & 0.883 & \textbf{0.863} & 0.844 & 0.728
       & \textbf{0.884} & 0.859 & \textbf{0.845} & \textbf{0.730} \\
Pinky  & 0.386 & 0.455 & 0.470 & 0.477
       & \textbf{0.790} & 0.762 & 0.768 & 0.681
       & 0.790 & \textbf{0.763} & \textbf{0.768} & \textbf{0.685} \\
\midrule
Wrist  & \multicolumn{4}{c}{0.204}
       & \multicolumn{4}{c}{0.847}
       & \multicolumn{4}{c}{\textbf{0.850}} \\
Mean   & \multicolumn{4}{c}{0.425}
       & \multicolumn{4}{c}{\textbf{0.819}}
       & \multicolumn{4}{c}{0.819} \\
Range  & \multicolumn{4}{c}{0.398}
       & \multicolumn{4}{c}{0.227}
       & \multicolumn{4}{c}{0.221} \\
\bottomrule
\multicolumn{13}{l}{\scriptsize Bold = best trained model per joint (GT annotations, uniform coord.\ space,}\\
\multicolumn{13}{l}{\scriptsize \S V-A; ties broken by unrounded value). FT~V2-150 leads on 12/21 joints,}\\
\multicolumn{13}{l}{\scriptsize Dist.\ V2-100 on 9/21. Mean and range (max$-$min) are over all 21 joints.}\\
\multicolumn{13}{l}{\scriptsize $^{*}$Superseded, uncorrected implementation (\S\ref{sec:quant}); not re-run for the}\\
\multicolumn{13}{l}{\scriptsize parity-controlled baseline in Table~\ref{tab:trained}.}
\end{tabular}%
}
\end{table}
...
input are provided in the code repository (\S I). Both are consistent
with the pattern above: overlay deviations cluster at Tip joints, and
webcam inference remains stable without domain-specific adaptation.
...
\parbox{\columnwidth}{\scriptsize $^{a}$All-frames scoring, fixed 51.2\,px
  threshold, full 2,727-sample test set (\S V-A); the MP~SDK row is a
  ...
  trained by any loss term (\S\ref{sec:quant}), so 100\% is not
...
$20{\times}$ slower into a student that remains fully deployable at
real-time speeds. Second, retraining through this pipeline yields an
open alternative to MediaPipe Hands reaching PCK@0.2\,=\,0.819 on
...
outperforms the framework by 12.6--12.7\,pp AUC once the earlier
baseline's coordinate-frame bug and missing regularization term are
corrected (\S\ref{sec:quant}): the previously reported advantage for
distillation reflected an incomplete baseline implementation, not a
property of the supervision signal itself. On this fully-labeled
synthetic benchmark, the framework's demonstrated value therefore
lies in compressing the teacher $4.8{\times}$ into a real-time
student without using ground-truth labels, not in exceeding a
well-trained direct-supervision baseline.

Per-joint, distillation improved all 21 evaluated joints, with its
largest gains at exactly the joints the superseded baseline
localized worst (wrist, MCP) and its smallest at Tip. The gradient
within the distilled models themselves (Tip lowest, MCP highest)
remains unexplained, motivating the kinematic and loss-weighting
directions below.

Two items follow directly from the central finding: the
direct-supervision baseline's hyperparameters, currently inherited
unchanged from the distillation-tuned recipe (\S\ref{sec:quant}),
should be tuned independently, and seed-variance estimates obtained
for the parity retrain, which is a single run; whether
distillation's advantage reappears under labeled-data scarcity --
which RHD2D's free synthetic ground truth does not exercise --
should also be tested. Two further items are low-cost and
self-contained: resolving the BGR/RGB channel-order confound
(\S\ref{sec:quant}) needs only one re-evaluation pass on the
existing checkpoints, not retraining, and the Tip-joint error
gradient (\S\ref{sec:perjoint}) merits a
properly instrumented, version-controlled teacher-heatmap
spatial-spread analysis, since the earlier sharpness hypothesis was
withdrawn as unvalidated rather than refuted. On the deployment
side, integrating a lightweight palm detector such as
BlazePalm~\cite{mediapipe2020} would complete an end-to-end pipeline
comparable to MediaPipe Hands, and post-training quantization or
mobile-runtime export (TFLite, ONNX Runtime, CoreML) would close the
CPU throughput gap. Finally, a zero-shot test of the RHD-trained
FT~V2-150 checkpoint on 1,628 real FreiHAND samples already reaches
PCK@0.2\,=\,0.757, AUC\,=\,0.719, MPJPE\,=\,36.0\,px with no
fine-tuning, motivating fuller evaluation on FreiHAND, OneHand10K,
and HO-3D.
