...
# RHD/teacher joint order is tip-first; MediaPipe is MCP-first.
...
print(f'MediaPipe SDK Comprehensive Eval — Teacher Coord Reference')
print(f'Model: Full (model_complexity=1, 1.98M params)')
print(f'Reference: HRNetV2-W18 teacher predictions')
...
for idx, batch in enumerate(loader):
    ...
    if (idx + 1) % 500 == 0:
        print(f'  [{idx+1}/{N}] Running PCK@0.2: {correct.sum()/((idx+1)*21):.4f} | '
              f'Det rate: {mp_detected/(idx+1)*100:.1f}%')

dists    = np.array(dists)
pck      = correct.sum() / (N * 21)
mpjpe    = dists.mean()
mse      = np.mean(np.sum(dists ** 2, axis=1) / 21)
det_rate = mp_detected / N * 100
...
print('Reference: HRNetV2-W18 teacher predictions (RHD-trained)')
...
print(f'Detected     : {mp_detected}/{N}')
print(f'Not detected : {mp_failed}/{N}')
print()
...
print('MediaPipe was trained on real hands, not synthetic RHD.')

with open('mediapipe_sdk_tc_results.txt', 'w') as f:
    f.write('Official MediaPipe SDK — Teacher Coord Evaluation\n')
    f.write('Model: Full (model_complexity=1, 1.98M params)\n')
    f.write('Reference: HRNetV2-W18 teacher predictions (RHD-trained)\n')
