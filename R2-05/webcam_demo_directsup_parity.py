...
# Repointed from webcam_demo_finetuned.py -- identical pipeline, only the
# checkpoint and on-screen labels differ (shared BlazeHandLandmark architecture).
CHECKPOINT = os.path.join(BASE, 'R2-05', 'runs', 'parity_baseline', 'checkpoints', 'parity_final.pth')
...
print('Loading R2-05 parity Direct Supervision student (v2match, epoch 100)...')
...
while True:
    ...
    if result.multi_hand_landmarks:
        for hand_lm in result.multi_hand_landmarks:
            ...
            with torch.no_grad():
                ...
                kps = landmarks[0, :, :2].cpu().numpy() * 256
    ...
    cv2.putText(frame, 'R2-05 Parity Direct Sup.  |  Epoch 100 (v2match)  |  21 hand keypoints',
                (10, fh - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (180, 180, 180), 1)

    cv2.imshow('Hand Keypoint Demo — R2-05 Parity Direct Supervision', frame)
    ...
    if key == ord('q'):
        ...
        saved += 1
        os.makedirs(os.path.join(BASE, 'R2-05', 'webcam_frames'), exist_ok=True)
        path = os.path.join(BASE, 'R2-05', 'webcam_frames', f'directsup_parity_{saved}.png')
