...
# RHD joint order is tip-first (thumb4=tip, thumb1=CMC, forefinger4=tip, etc.)
...
for idx, batch in enumerate(loader):
    ...
    # Ground truth keypoints from RHD annotations (in 256x256 transformed space)
    gt_kpts = batch['data_samples'][0].gt_instances.keypoints[0]  # (21, 2)
    ...
    if result.multi_hand_landmarks:
        ...
        gt_ordered = gt_kpts[MP_TO_RHD]  # reorder GT to match MediaPipe joint order
        dist = np.sqrt(np.sum((mp_coords - gt_ordered)**2, axis=1))
...
print()
print('='*60)
print('OFFICIAL MEDIAPIPE SDK — RHD Test Set (vs Ground Truth)')
...
print('='*60)
print()
print('NOTE: Low detection rate expected — MediaPipe was trained')
print('on real hands, not synthetic RHD colored-glove images.')

with open('mediapipe_sdk_results.txt', 'w') as f:
    f.write('Official MediaPipe SDK Evaluation (vs Ground Truth)\n')
...
print('Saved: mediapipe_sdk_results.txt')