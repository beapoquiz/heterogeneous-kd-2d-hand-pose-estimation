"""
TEACHER EVAL — FILE A (FIXED)
Teacher vs Ground Truth, standard decode path, correct coordinate space.

FIX: Both predictions (pred_instances.keypoints) and GT keypoints are
     kept in the original image space. GT is loaded directly from the
     RHD annotation file — NOT from gt_instances.keypoints, which is
     in 256x256 crop space after TopdownAffine.

Should reproduce official tools/test.py result:
     PCK@0.2 ≈ 0.9918 | AUC ≈ 0.9023 | EPE ≈ 2.18 px

Run from project root:
    python teacher_eval_A_fixed.py
"""

import os, json
import numpy as np
import torch
from mmpose.apis import init_model
from mmpose.datasets import Rhd2DDataset
from torch.utils.data import DataLoader
from mmengine.dataset import pseudo_collate
from mmengine.registry import init_default_scope

init_default_scope('mmpose')

BASE   = os.path.dirname(os.path.abspath(__file__))
CONFIG = os.path.join(BASE, 'mmpose/configs/hand_2d_keypoint/topdown_heatmap/'
                      'rhd2d/td-hm_hrnetv2-w18_dark-8xb64-210e_rhd2d-256x256.py')
CKPT   = os.path.join(BASE, 'checkpoints',
                      'hrnetv2_w18_rhd2d_256x256_dark-4df3a347_20210330.pth')
BATCH  = 32
AUC_NORM  = 30.0
AUC_STEPS = 20

# ── Load raw GT annotations (original image space) ────────────────────────────
ANN_FILE = os.path.join(BASE, 'dataset', 'rhd', 'annotations', 'rhd_test.json')
with open(ANN_FILE) as f:
    coco = json.load(f)

# Map image_id → annotation
ann_by_imgid = {}
for ann in coco['annotations']:
    ann_by_imgid[ann['image_id']] = ann

# Map image_id → image info (width, height)
img_by_id = {img['id']: img for img in coco['images']}

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print(f'Device : {device}')
print('Loading teacher (standard decode — pred_instances.keypoints)...')
teacher = init_model(CONFIG, CKPT, device=device)
teacher.eval()

pipeline = [
    dict(type='LoadImage'),
    dict(type='GetBBoxCenterScale'),
    dict(type='TopdownAffine', input_size=(256, 256)),
    dict(type='PackPoseInputs'),
]
ds = Rhd2DDataset(
    data_root=os.path.join(BASE, 'dataset', 'rhd'),
    ann_file='annotations/rhd_test.json',
    pipeline=pipeline,
)
loader = DataLoader(ds, batch_size=BATCH, shuffle=False,
                    num_workers=0, collate_fn=pseudo_collate)
print(f'Test set : {len(ds)} samples | batch={BATCH} | {len(loader)} batches\n')

# ── single pass ────────────────────────────────────────────────────────────────
kp_dists   = []
bbox_sizes = []
img_epes   = []

for b_idx, batch in enumerate(loader):
    img_tensor = torch.stack(batch['inputs']).float() / 255.0

    with torch.no_grad():
        results = teacher.predict(img_tensor.to(device), batch['data_samples'])

    for res, ds_sample in zip(results, batch['data_samples']):
        # Predicted keypoints — in original image space (predict() applies inverse affine)
        pred_kps = np.array(res.pred_instances.keypoints[0]).reshape(21, 2)

        # GT keypoints — load from raw annotation (original image space)
        try:
            img_id = ds_sample.img_id
        except AttributeError:
            img_id = ds_sample.metainfo.get('img_id', None)

        if img_id not in ann_by_imgid:
            continue

        ann      = ann_by_imgid[img_id]
        raw_kps  = ann['keypoints']                   # list of [x, y, v] × 21
        gt_kps   = np.array([[kp[0], kp[1]] for kp in raw_kps], dtype=float)
        gt_vis   = np.array([kp[2] for kp in raw_kps], dtype=float)

        # Bbox size from annotation (original image space)
        bbox      = ann['bbox']                        # [x, y, w, h]
        bbox_size = max(bbox[2], bbox[3])

        vis = gt_vis > 0
        if vis.sum() == 0:
            continue

        dists = np.linalg.norm(pred_kps - gt_kps, axis=1)[vis]
        kp_dists.extend(dists.tolist())
        bbox_sizes.extend([float(bbox_size)] * int(vis.sum()))
        img_epes.append(float(dists.mean()))

    print(f'  {min((b_idx+1)*BATCH, len(ds)):4d}/{len(ds)}...', end='\r')

print()

kp_dists   = np.array(kp_dists)
bbox_sizes = np.array(bbox_sizes)
img_epes   = np.array(img_epes)
thr_bbox   = bbox_sizes * 0.2
thrs       = np.linspace(0, AUC_NORM, AUC_STEPS + 1)[1:]

epe       = float(kp_dists.mean())
pck_bbox  = float((kp_dists < thr_bbox).mean())
pck_fixed = float((kp_dists < 0.2 * 256).mean())
auc       = float(np.mean([(kp_dists < t).mean() for t in thrs]))

print()
print('=' * 58)
print('  TEACHER — FILE A (FIXED)  pred_instances vs original GT')
print('=' * 58)
print(f'  EPE            : {epe:.4f} px   (official: ~2.18 px)')
print(f'  PCK@0.2 [bbox] : {pck_bbox:.4f}      (official: ~0.9918)')
print(f'  PCK@0.2 [fixed]: {pck_fixed:.4f}      (0.2×256 = 51.2 px)')
print(f'  AUC  [0-30 px] : {auc:.4f}      (official: ~0.9023)')
print('=' * 58)
print()
print('Bbox size stats (original image space):')
print(f'  Median bbox_size  : {np.median(bbox_sizes):.1f} px')
print(f'  Median PCK thr    : {np.median(thr_bbox):.1f} px  (bbox × 0.2)')
print(f'  Fixed thr         : {0.2*256:.1f} px')
print()
print('Per-image EPE distribution:')
for lo, hi in [(0,2),(2,5),(5,10),(10,20),(20,9999)]:
    n   = int(((img_epes >= lo) & (img_epes < hi)).sum())
    pct = 100 * n / len(img_epes)
    hi_str = f'{hi}' if hi < 9999 else '∞'
    print(f'  [{lo:2d}–{hi_str:>4s} px]: {n:5d} imgs  ({pct:5.1f}%)  '
          + '█' * int(pct / 2))

print('\nDone — File A (fixed).')
